...
export default function WorkflowDetailPage({ workflowId }: Props) {
  ...
  const handleRun = async () => {
    ...
    const loadingToastId = toast.loading('Starting workflow...', {
      description: 'Please wait while the workflow is being executed',
    });

    try {
      const response = await fetch(`http://localhost:3000/workflow/${workflowId}/run`, {
        method: 'POST',
        headers: {
      ...
      toast.success('Workflow started successfully!', {
        description: result.message || 'The workflow is now running.',
      });
      ...
      <div className="card p-12 text-center max-w-lg mx-auto mt-12">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-xs text-muted-foreground">
            {error || 'Could not find the requested workflow'}
          </p>
    ...
    <div className="p-6 max-w-screen-2xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={fetchWorkflow} className="btn-secondary text-xs py-1.5">
          ...
          {workflow.status === 'ACTIVE' && (
            <button onClick={handleRun} className="btn-primary text-xs py-1.5">
              <PlayIcon width={14} height={14} />
              Run
            </button>
          )}
            ...
            <div className="card p-5 space-y-3">
              ...
              <div className="space-y-2.5">
                ...
                <div>
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium border ${statusBadge.bg} ${statusBadge.text} ${statusBadge.border}`}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${statusBadge.dot}`} />
                    {workflow.status}
                  </span>
              ...
              <div className="space-y-2.5">
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">
                    {getScheduleTypeLabel(workflow.scheduleType)}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">
